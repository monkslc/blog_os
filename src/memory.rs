...
/// A FrameAllocator that returns usable frames form the bootloader's memory map
...
/// mapped to virtual memory at the passed `physical_memeory_offset`. Also, this function must be
...
///
/// This function is usnafe because the caller must guarantee that the complete physical memory is
/// mapped to virtual memroy at the passed `physical_memory_offset`. Also, this function must be
/// only called once to avoid aliasing `&mut` references (which is undefined behavior).
